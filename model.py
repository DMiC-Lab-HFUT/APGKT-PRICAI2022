...
class GIKT(object):
    def __init__(self, args):
        ...
        self.args = args
        self.hidden_neurons = args.hidden_neurons  # '--hidden_neurons', type=int, default=[200,100]
        self.max_step = args.max_step - 1  # 199
        self.feature_answer_size = args.feature_answer_size
        self.field_size = args.field_size
        self.embedding_size = args.embedding_size

        self.dropout_keep_probs = eval(args.dropout_keep_probs)
        self.select_index = args.select_index  # "--select_index", type=int, default=[0, 1, 2]
        self.hist_neighbor_num = args.hist_neighbor_num  # M  arg_parser.add_argument('--hist_neighbor_num', type=int, default=0)  # history neighbor num
        self.next_neighbor_num = args.next_neighbor_num  # N
        self.lr = args.lr
        self.n_hop = args.n_hop  # '--n_hop', type=int, default=3
    ...
    def build_model(self):
        ...
        self.input_questions_embedding = tf.nn.embedding_lookup(self.feature_embedding,
                                                                questions_index)  # [batch_size,seq_len,d]  (?,199,100)
        self.next_questions_embedding = tf.nn.embedding_lookup(self.feature_embedding,
                                                               next_questions_index)  # [batch_size,seq_len,select_size-1,d]  (?,199,100)
        ...
        input_answers_embedding = tf.nn.embedding_lookup(self.feature_embedding,
                                                         select_feature_index[:, :-1, -1])
        input_answers_index = select_feature_index[:, :-1, -1]

        if self.n_hop > 0:  # '--n_hop', type=int, default=3
            # gnn
            ...
            feature_emb_size = self.embedding_size  # 100
            feature_trans_embedding = tf.reshape(
                tf.layers.dense(tf.reshape(aggregate_embedding[0], [-1, feature_emb_size]), hidden_size,
                                # aggregate_embedding[0] (?,199,?,100)
                                activation=tf.nn.relu, name='feature_layer', reuse=False),  # reuse=None
        ...
        input_fa_embedding = tf.reshape(tf.concat([feature_trans_embedding, input_answers_embedding], -1),
                                        # feature_trans_embedding(?,199,100)  input_answers_embedding (?,199)
                                        [-1, hidden_size + self.embedding_size])  # embedding_size*2  (?,200)
        input_trans_embedding = tf.reshape(tf.layers.dense(input_fa_embedding, hidden_size),
                                           # input_fa_embedding(?,200), hidden_size(100)
                                           [-1, self.max_step, hidden_size])  # (?,199,100) self.max_step=199
        ...
        for idx, hidden_size in enumerate(self.hidden_neurons):
            ...
            hidden_layers_1.append(hidden_layer)
        self.hidden_cell_1 = tf.contrib.rnn.MultiRNNCell(cells=hidden_layers_1, state_is_tuple=True)  # RNN

        output_series_1 = []
        states_series_1 = []

        self.state_1 = self.hidden_cell_1.zero_state(self.batch_size, tf.float32)

        with tf.variable_scope('rnn_layer1'):
            for i in range(self.max_step):
                current_output_1, self.state_1 = self.hidden_cell_1(input_trans_embedding[:, i, :], self.state_1)
                output_series_1.append(current_output_1)  # (？*100)
                states_series_1.append(self.state_1[-1].h)

        output_series_1 = tf.reshape(tf.concat(output_series_1, 1),
                                     [-1, self.max_step, hidden_size])  # tf.concat(output_series_1, 1 ==> ?*1990

        self.Q_modes = tf.convert_to_tensor(self.Q_modes)
        self.input_modes_embedding = tf.nn.embedding_lookup(self.Q_modes, questions_index)
        hidden = tf.layers.dense(self.input_modes_embedding, hidden_size, activation=tf.nn.relu,
                                 kernel_regularizer=tf.contrib.layers.l2_regularizer(0.003))

        self.input_modes_embedding = tf.reshape(hidden, [-1, self.max_step, hidden_size])
        outputs = tf.layers.dense(hidden, hidden_size, activation=tf.nn.relu,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(0.003))

        reconstruction_loss = tf.reduce_mean(tf.square(outputs - self.input_modes_embedding))  # MSE

        input_layer2_embedding = tf.reshape(self.input_modes_embedding,
                                            [-1, hidden_size])

        input_layer2_embedding = tf.reshape(tf.layers.dense(input_layer2_embedding, hidden_size),
                                            # input_fa_embedding(?,200), hidden_size(100)
                                            [-1, self.max_step, hidden_size])  # (?,199,100) self.max_step=199
        ...
        for idx, hidden_size in enumerate(self.hidden_neurons):
            ...
            hidden_layer = tf.contrib.rnn.DropoutWrapper(cell=lstm_layer,
                                                         output_keep_prob=self.keep_prob)
            hidden_layers_2.append(hidden_layer)
        self.hidden_cell_2 = tf.contrib.rnn.MultiRNNCell(cells=hidden_layers_2, state_is_tuple=True)  # RNN

        output_series_2 = []
        states_series_2 = []

        self.state_2 = self.hidden_cell_2.zero_state(self.batch_size, tf.float32)

        with tf.variable_scope('rnn_layer2', reuse=False):
            for i in range(self.max_step):
                current_output_2, self.state_2 = self.hidden_cell_2(input_layer2_embedding[:, i, :], self.state_2)
                output_series_2.append(current_output_2)  # (？*100)
                states_series_2.append(self.state_2[-1].h)

        output_series_2 = tf.reshape(tf.concat(output_series_2, 1),
                                     [-1, self.max_step, hidden_size])  # tf.concat(output_series_1, 1 ==> ?*1990

        output_series_3 = tf.reshape(tf.concat([output_series_1, output_series_2], -1),
                                     [-1, hidden_size + self.embedding_size])

        output_series_3 = tf.reshape(tf.layers.dense(output_series_3, hidden_size),
                                     # input_fa_embedding(?,200), hidden_size(100)
                                     [-1, self.max_step, hidden_size])  # (?,199,100) self.max_step=199
        ...
        if self.args.model == "hssi":
            self.hist_neighbors_features = self.hist_neighbor_sampler(
                output_series_3)  # [self.batch_size,max_step,self.hist_neighbor_num,hidden_size]
        ...
        if self.hist_neighbor_num != 0:
            ...
            Nh = tf.concat([tf.expand_dims(output_series_3, 2), self.hist_neighbors_features],
            ...
            Nh = tf.expand_dims(output_series_3, 2)  # [self.batch_size,max_step,1,feature_trans_size]
        ...
        self.binary_pred = tf.cast(tf.greater_equal(self.pred, 0.5),
                                   tf.int32)
        ...
        self.loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=clear_flat_target_correctness,
                                                                           logits=clear_flat_target_logits)) + reconstruction_loss
